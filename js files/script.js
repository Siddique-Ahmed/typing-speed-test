...
function randomParagraph() {
  ...
  // getting random item from the array, splitting all characters of it, adding  each character inside span and then adding this span inside p tag \\
  paragraphs[randIndex].split("").forEach((span) => {
    let spanTag = `<span>${span}</span>`;
    typingText.innerHTML += spanTag;
  });
  typingText.querySelectorAll("span")[0].classList.add("active");
  // focus input field when keydown or click event \\
  ...
  if(charIndex < characters.length -1 && timeLeft > 0){
    ...
    if (typedCharacter == null) {
      ...
      if (characters[charIndex].innerText === typedCharacter) {
        // if user typed character and shown character matched then add the correct class else increament mistake and  add the incorrect class \\
